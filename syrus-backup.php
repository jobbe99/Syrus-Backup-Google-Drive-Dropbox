<?php
/*
Plugin Name: Syrus Backup
Version: 1.0
Plugin URI: http://www.syrusindustry.com/
Description: Back up your WordPress website to Dropbox or Google Drive.
...
if ( ! class_exists( 'GOAuth' ) )
	require_once( 'helpers/class-goauth.php' );
if ( ! class_exists( 'GDocs' ) )
	require_once( 'helpers/class-gdocs.php' );
// Load helper functions
require_once( 'helpers/functions.php' );

/**
 * Backup Class.
 * Implements backup functionality in WordPress.
 * Currently supports backing up to Dropbox and to Google Drive.
 */
class Backup {
	/**
	...
	/**
	 * Constructor
	 * This loads most of what's needed to run the plugin
	 * and sets actions and filters.
	 */
	function __construct() {
		...
		if ( ! $this->options = get_option( 'backup_options' ) ) {
			$this->options = array(
				...
				'drive_folder'        => '',
				'dropbox_folder'      => '',
				'dropbox_key'         => 's8p4jodpbbwl3zm',
				'dropbox_secret'      => 'vhhltgm7p6b9vzq',
		...
		if (defined('BACKUP_DROPBOX_FOLDER')) $this->options['dropbox_folder'] = BACKUP_DROPBOX_FOLDER;
		 ...
		 * Setting up dropbox
		 */
		// if ( defined( 'BACKUP_DROPBOX_FOLDER' ) && isset( $this->options['dropbox_key'] ) && isset( $this->options['dropbox_secret'] ) )
		// 	//call dropbox
 	...
 	function options_update() {
 		if ( isset( $_GET['action'] ) && 'update' == $_GET['action'] ) {
			...
			} else if ( isset( $_POST['dropbox_number'] ) ) {
				$dropbox_number = absint( $_POST['dropbox_number'] );
			}
			if ( 0 == $local_number && 0 == $drive_number && 0 == $dropbox_number)
				$this->messages['error'][] = __(
					'You need to store at least one local, Dropbox or Drive backup.', $this->text_domain
				);
			else {
				$this->options['drive_number']   = $drive_number;
				$this->options['local_number']   = $local_number;
				$this->options['dropbox_number'] = $local_number;
			}
			...
			// Save Dropbox folder.
 			if ( !defined( 'BACKUP_DROPBOX_FOLDER' ) && isset( $_POST['dropbox_folder'] ) )
 				$this->options['dropbox_folder'] = $_POST['dropbox_folder'];
